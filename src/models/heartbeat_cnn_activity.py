...
def cli_main():
    ...
    #sys.stdout = open("output_perActivity.txt", "w")
    ...
    #data_path = "../data/perSubject_Activity/"
    data_path = "../../data/interim/PPG_FieldStudy_CNN_Input_byActivity/"
    ...
    # extend labels list for each activity
    labels = [item for item in labels for i in range(9)]
    # delete those that do not have a match in S6 - Activities 6 through 8
    # the paper explains that the last approximately one hour of the data
    # collection protocol is missing for subject S6
    del labels[51:54]
    ...
    for f, l in zip(files, labels):
        ...
        mae_list[str(x['current_subject'].iloc[0])] = mae
    ...
    print('Execution time:' + str(runT) + 'seconds')
